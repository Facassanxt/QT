...
public:
    ...
    bool swap()
    {
        streamF.open("E:\\Final\\test.txt", ios::out | ios::trunc);
        upload();
        closeFile();
        return true;
    }
    ...
    void fileToVector(){
       ...
       ifstream streamF("E:\\Final\\"+_tableName+".txt");
...
string rez;
    ...
    string ssearch(string k, string v){ //тип значения поля - string
        ...
        for (unsigned i = 0; i < _db.size(); i++)
        {
            ...
            if(k == "name" && item.name == v){
                select.push_back(item);
                rez = rez + item._id +":"+ item.name +":"+ item.year +":"+ item.address +":"+ item.tel +":"+ item.login +":"+ item.password +"\n";
            }
