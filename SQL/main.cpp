...
int main(int argc, char *argv[])
{
    ...
    // Соединяемся с менеджером баз данных
     ...
     QString str = "CREATE TABLE IF NOT EXISTS addressbook (ID INTEGER PRIMARY KEY AUTOINCREMENT,  name VARCHAR(15) NOT NULL,  phone VARCHAR(12) NOT NULL,  email VARCHAR(15) NOT NULL);";
...
//      }
     ...
     if(!query.exec("SELECT count(*), phone, email FROM addressbook  WHERE name = 'Piggy'; ")){
         qDebug() << "Unable to execute query — exiting";
              return 1;
     }
     QSqlRecord rec = query.record();
     ...
     QString countt;
           ...
           while (query.next()) {
                ...
                countt = query.value(rec.indexOf("count(*)")).toString();
           strPhone = query.value(rec.indexOf("phone")).toString();
           strEmail = query.value(rec.indexOf("email")).toString();

                 qDebug() << countt<< "\t" << strPhone << "\t" << strEmail;
                 }
